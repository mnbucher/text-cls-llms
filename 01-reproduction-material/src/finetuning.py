...
def init_misc(rand_seed, run_id, is_debug, remove_log_files=False):
	set_seeds(rand_seed)
...
def print_failed_classifier_majority(y_all, rand_seed, dataset_name = ""):
	...
	compute_classification_metrics(y_all, y_pred, None, f"majority-voting-{dataset_name}", rand_seed, None, True)
	print("")



def resample_dataset(x, y, imbalance_strategy, ds, is_debug, dataset_name = ""):
...
def resample_datasets(x_train, x_val, y_train, y_val, imbalance_strategy, run_id, rand_seed, do_validation_set, is_debug, dataset_name = ""):
	...
	if is_debug: print_failed_classifier_majority(y_all, rand_seed, dataset_name = f"{run_id}")
	...
	x_train_resampled, y_train_resampled = resample_dataset(x_train, y_train, imbalance_strategy, f"train-{run_id}", is_debug)

	if do_validation_set:
		x_val_resampled, y_val_resampled = resample_dataset(x_val, y_val, imbalance_strategy, f"val-{run_id}", is_debug)
...
def get_embeddings_from_sentences(x_all, max_seq_length, language_model, language_for_model, custom_model_name, is_debug):
	...
	if is_debug: print("check if samples will get truncated...")
	...
	if is_debug: print(f"number of samples that will get truncated: {num_truncated_samples}")
...
def compute_classification_metrics(y_true, y_pred, epoch, dataset, rand_seed, log_with_wandb, is_debug, dataset_name = ""):

	prec_score, f1_score_weighted, f1_score_macro, acc_score, rec_score = get_metrics(y_true, y_pred, dataset, is_debug)

	file_name = f"./output/results-{dataset}-{dataset_name}-{rand_seed}.csv"
...
def compute_metrics_from_onehot_y(y_pred_enc, y_true_enc, loss_all, dataset, epoch, log_with_wandb, y_onehot_encoder, validation_set_metric, rand_seed, is_debug, dataset_name = ""):
	...
	prec_score, f1_score_weighted, f1_score_macro, acc_score, rec_score = compute_classification_metrics(y_true_dec, y_pred_dec, epoch, dataset, rand_seed, log_with_wandb, is_debug, dataset_name = dataset_name)
...
def compute_and_print_metrics_for_dataset_b(y, y_pred, epoch, rand_seed, log_with_wandb, is_debug, dataset_name = ""):

	if(type(y_pred) is not list):
		compute_classification_metrics(y, y_pred, epoch, "test", rand_seed, log_with_wandb, is_debug, dataset_name = dataset_name)
	else:
		y_preds = y_pred
		...
		acc_fmt = f"accuracy: {round(np.mean(acc_scores), 3)} +/- {round(np.std(acc_scores), 3)}"
		prec_fmt = f"precision: {round(np.mean(prec_scores), 3)} +/- {round(np.std(prec_scores), 3)}"
		rec_fmt = f"recall: {round(np.mean(rec_scores), 3)} +/- {round(np.std(rec_scores), 3)}"
		f1_macro_fmt = f"f1 score macro: {round(np.mean(f1_scores_macro), 3)} +/- {round(np.std(f1_scores_macro), 3)}"
		f1_weighted_fmt = f"f1 score weighted: {round(np.mean(f1_scores_weighted), 3)} +/- {round(np.std(f1_scores_weighted), 3)}"

		with open("stats.txt", "w") as f:
			f.write(acc_fmt + "\n" + prec_fmt + "\n" + rec_fmt + "\n" + f1_macro_fmt + "\n" + f1_weighted_fmt)
        
		file_name = f"./output/results-test-{dataset_name}-combined.csv"
...
def run_train_epoch(epoch, rand_seed, model, optimizer, gradient_accumulation_steps, imbalance_strategy, class_weights, scheduler, train_dataloader, train_n_total, n_classes, y_label_transformer, validation_set_metric, log_with_wandb, dvc, is_debug, dataset_name = ""):
	...
	compute_metrics_from_onehot_y(y_pred_all, y_true_all, loss_batchwise, "train", epoch, log_with_wandb, y_label_transformer, validation_set_metric, rand_seed, is_debug, dataset_name = dataset_name)

	empty_cuda_cache()


def run_val_epoch(epoch, rand_seed, model, imbalance_strategy, class_weights, val_dataloader, val_n_total, n_classes, y_label_transformer, validation_set_metric, log_with_wandb, dvc, is_debug, dataset_name = ""):
	...
	with torch.no_grad():
		...
		val_metric, y_pred_dec = compute_metrics_from_onehot_y(y_pred_all, y_true_all, loss_batchwise, "val", epoch, log_with_wandb, y_label_transformer, validation_set_metric, rand_seed, is_debug, dataset_name = dataset_name)
...
def run_training(n_epochs, run_id, rand_seed, model, optimizer, imbalance_strategy, class_weights, scheduler, train_dataloader, train_n_total, val_dataloader, val_n_total, n_classes, y_label_transformer, do_validation_set, validation_set_metric, log_with_wandb, dvc, language_model, language_for_model, custom_model_name, max_seq_length, dataset_B_unlabelled_x, batch_size, gradient_accumulation_steps, is_debug):
	...
	for epoch in range(n_epochs):
		...
		run_train_epoch(epoch, rand_seed, model, optimizer, gradient_accumulation_steps, imbalance_strategy, class_weights, scheduler, train_dataloader, train_n_total, n_classes, y_label_transformer, validation_set_metric, log_with_wandb, dvc, is_debug, dataset_name = f"{run_id}")
